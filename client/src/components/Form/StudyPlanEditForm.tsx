...
const StudyPlanEditForm = () => {
  ...
  useEffect(() => {
    let isMounted = true;
    const checkInfo = {
      studyplanTitle: studyplan.studyplanTitle,
      endDate: studyplan.endDate,
      selected: studyplan.selected,
    };
  ...
  useEffect(() => {
    getSubject(user.id);
    if (params.subjectId)
      setStudyplan({ ...studyplan, selected: params.subjectId });
  ...
  async function getInfo(planId:string) {
    try {
      setError("");
  ...
  function handleChange(e:) {
    ...
    if (name === "deadline") {
      studyplan.deadline !== "on"
        ? setStudyplan({ ...studyplan, [name]: value })
        : setStudyplan({ ...studyplan, [name]: "off" });
    } else {
      setStudyplan({ ...studyplan, [name]: value });
    }
  }
  ...
  async function handleSubmit(e:) {
    ...
    try {
      ...
      if (params.subjectId) {
        //등록함수 실행
        await planApi.submit(studyplan);
  ...
  return (
    <div className="pt-5">
      <select
        className={`form-select mb-4 ${styles.selectInput}`}
        name="selected"
        ...
        {subjectList.length !== 0 ? (
          subjectList.map((subject: any, idx: number) => {
            if (idx === 0)
              return (
                <>
                  <option selected={studyplan.selected === "" ? true : false}>
                    관련 과목을 선택해주세요.
                  </option>
                  <option
                    value={subject.id}
                    selected={studyplan.selected === subject.id ? true : false}
                  >
                    {subject.name}
                  </option>
                </>
      ...
      <input
        ...
        placeholder="제목"
        value={studyplan.studyplanTitle}
        onChange={handleChange}
        autoComplete="off"
      />
      <div className="d-flex mb-3">
        <label className="col col-form-label align-self-center py-0">
          마감일
        </label>
        <div className={studyplan.deadline === "on" ? "col-5" : "col-7"}>
          <input
            className={`form-control shadow-none ${styles.dateInput}`}
            type="date"
            name="endDate"
            aria-label="endDate"
        ...
        <div
          className={
            "col-4 " + (studyplan.deadline === "on" ? "d-block" : "d-none")
          }
        >
          <input
            className={`form-control shadow-none ${styles.dateInput}`}
      ...
      <div className="d-flex justify-content-end form-check mb-4">
        <input
          className={`form-check-input shadow-none ${styles.checkBox} me-2`}
          type="checkbox"
          id="deadline"
          name="deadline"
          onChange={handleChange}
          checked={studyplan.deadline === "on" ? true : false}
        />
